...
def convert_wav_to_csv(wav_file, csv_file):
    # Read the WAV file
    ...
    # Convert audio data to digits
    digits = [str(x) for x in audio_data]
    
    # Save digits to CSV file
    with open(csv_file, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Digit'])
        writer.writerows(map(lambda x: [x], digits))

# example usage#############################################################
wav_file = 'input.wav'  # Path to your WAV file
csv_file = 'output.csv'   # Path to save CSV file
convert_wav_to_csv(wav_file, csv_file)
